...
def open_door(persons):
	print("Door opened for {}".format(persons))
...
try:
    ...
    #         pixels = np.asarray(img)
        ...
        for (x, y, w, h) in faces:
            ...
            if prev_face=="" and count==1 and predict_names!="Unknown":
            	prev_face = predict_names
            	counter = 1
            elif count==1 and counter<15 and prev_face==predict_names:
            	counter+=1
            elif count==1 and counter==10 and prev_face==predict_names:
            	open_door(predict_names)
            	counter=0
            	prev_face=""
...
def xml_to_csv(path):
	for xml_file in glob.glob(path + '/*.xml'):
        ...
        root = tree.getroot()
        for member in root.findall('object'):
        	if int(member[4][2].text)<int(member[4][0].text) or int(member[4][3].text)<int(member[4][1].text):
